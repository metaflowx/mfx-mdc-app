...
const ClaimButton: React.FC<ClaimButtonProps> = ({ isClaimChecking, level }) => {
  const {
    writeContractAsync: writeContractAsyncClaim,
    isPending: isPendingClaim,
    isSuccess,
  ...
  return (
    <Button
      ...
      startIcon={
        isPendingClaim ? <CircularProgress color="inherit" size={18} /> : null
      }
    >
      {isPendingClaim ? "Processing..." : "Claim"}
    </Button>
  );
};
